...
**2026-07-10 真實資料結果(TWSE+TPEx 1,994 檔、607 交易日、107 對齊週;因子=OpenRouter token 單腿 + SOX 殘差;詳 reports/real_results.json)**

| 假說 | 真實資料結論 | 依據 |
|---|---|---|
| H1 消費因子在台股溢酬=0 | ✅ **與 H1 一致(複製論文 EM 發現)** | VW H−L = **−0.4 bps/週**(t=−0.02),95% CI **[−41, +40]** → 點估計精確為零,且 CI **排除論文 DM 量級 64.1 bps**;FM λ 不顯著;五分位無單調性;動能糾纏乾淨(Q1 0.018 vs Q5 0.019)。受 E2P 檢定力天花板約束,結論寫成區間:「若有溢酬,95% 信賴上界 +40 bps/週」 |
| H2 供給端因子有正溢酬 | ❌ **未獲支持** | SOX 殘差腿 H−L = +16.4 bps/週(t=0.68),CI [−31, +63]。方向為正但遠不顯著。按事前證偽條件:「供給端因子」假說**作廢不硬凹**;註:因子為降級代理(缺 hyperscaler guidance 腿) |
| H3 外資流中介 | ❌ **不成立** | (a) 雙重排序:溢酬在任何外資流組皆不存在(買超格 −6.9 bps t=−0.28;買−賣差 −15.2 t=−0.56);(b) FM 加流量後 λ 87.8→102.1,無衰減;(c) 通道拆解:隔夜 −23.5 (t=−1.3) vs 盤中 +18.5 (t=1.0) → 若有任何定價是**本地盤中**,非 ADR 夜盤傳導,方向與 H3 相反 |
| Design 1 事件研究(17 前沿事件) | 🟡 弱正、集中於盤中 | 五日 H−L = +0.281%(t=1.26, boot p=0.20)不顯著;拆解:隔夜 −0.04%(零)、**盤中 +0.355%(t=1.74, boot p=0.074)** 邊際;非前沿安慰劑 total 乾淨(−0.06%)。SOX-beta 分組下全部消失(R1b)→ 訊號依賴消費 beta。異常註記:安慰劑的隔夜通道 −0.315%(t=−3.44)顯著為負,無事前假說,列為待解釋的探索性觀察 |

**合成資料管線驗證(第一輪,詳 reports/experiment_report.md)**:E1 事件研究 size/power/通道拆解全通過;E2P 檢定力:107–130 週下 EM 量級 power<10%、DM 量級 38%(降級因子 29%)→ 一切「不顯著」判定皆受此天花板約束;E3 FM 衰減比率指標不穩定已棄用。

**新增資料品質事實**:TWSE rwd API 在限流時會回**錯誤日期的資料**(收到 2017-12-18 的表),已以「內嵌日期 vs 檔名日期」檢核抓出 5 檔並重抓;解析器已加去重防護。此 quirk 寫給未來的自己。
